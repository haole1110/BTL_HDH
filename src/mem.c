	...
	if (mem_avail) {
		...
		for (int i=0; i<num_pages; i++){
			if(_mem_stat[index].proc == 0) {
				...
				uint32_t vir_address = ret_mem + i * PAGE_SIZE;
				uint32_t first_layer = get_first_lv(vir_address);
				uint32_t second_layer = get_second_lv(vir_address);

				if (get_trans_table(first_layer, proc->seg_table) == NULL){
					proc->seg_table->table[proc->seg_table->size].v_index = first_layer;
					struct page_table_t* temp_page = (struct trans_table_t*)malloc(sizeof(struct trans_table_t));
					temp_page->size = 0;
					proc->seg_table->table[proc->seg_table->size].pages = temp_page;
					proc->seg_table->size++;
				}
				struct page_table_t* temp_page = get_trans_table(first_layer, proc->seg_table);
				temp_page->table[temp_page->size].v_index = second_layer;
